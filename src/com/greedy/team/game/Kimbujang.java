...
public class Kimbujang extends EscapeDTO {
	
	
	String daeri = ">대리운전";
	int minusHp = 30;	//	각 캐릭터 마다 한번 술 마실 때 감소되는 체력 설정 값  
	int pluseHp = 40;	//	각 캐릭터 마다 안주를 한번 먹을 때 마다 증가되는 체력 설정 값 
	...
	public void recharge() {
		if(this.hp < 100){
			...
			System.out.println("김부장의 체력은 " + hp + "%입니다.");
		}else {
	...
	public void drink() {
		...
		if(hp >= 50) {
			System.out.println("김부장이 소주를 마십니다. 크으~~ ");
			hp -= minusHp;
			count++; //술을 몇잔 마셨는지 카운트
			System.out.println("김부장은 현재 소주 " + count + "잔을 마셨습니다.");
			System.out.println("김부장의 남은 체력은 " + hp + "%입니다.");
			...
			if(mission == count) {
				...
				while(true) {
					...
					System.out.println("\n[미션성공!!!!!]");
					System.out.print("소주를 다 마셨습니다. 대리기사를 부르시겠습니까? (y/n) : ");
						...
						for(int i = 0; i < 17; i++) // Str[50-3*i] 값이 음수가 되면 오류
						{
							...
							if(num == 68 | num == 100)
							{
								...
								System.out.print(iarr[i] = hof + daeri + Str[50-3*i] + "집");// Str의 공백의 수를 줄여서
																							...
																							// 대리운전이 세칸씩 움직이므로 -3씩 곱해줌
								System.out.println();
								System.out.println("부릉부릉 =3 열심히 달리는 중 입니다! 'D' 를 계속 눌러서 김부장을 빨리 귀가 시켜주세요! ");
							}															     
						}
							System.out.println();
							System.out.println("김부장이 대리기사가 운전해준 차를타고 무사히 집으로 귀가했습니다. 안녕히가세요.");
							System.exit(0);
					
					}	
					
						else if (answer.equalsIgnoreCase("n")) {
						System.out.println("대리기사를 못잡은 김부장은 해가 뜰 때 까지 술을 마셨습니다..! 게임을 종료합니다.");
						System.exit(0);
					}else {
		...
		// 체력이 50미만으로 떨어졌을 경우 술을 마실 수 없음 
			...
			System.out.println("김부장이 기운이 없어 소주를 마실 수 없습니다. 안주를 먹거나 게임을 이겨 체력을 충전하세요!");
			System.out.println("편의점에 상품 구매로도 체력을 충전할 수 있습니다!");
		}
	}
	//게임하
		...
		while (true) {
			...
			for (int i = 1; i <= 7; i++) {
				...
				if (userNum > computerNum) {
					...
					hp += 100;
					coin += 2000;
					System.out.println("축하합니다!! 김부장의 체력이 100% 충전 되었습니다! >_< ");
					System.out.println("축하합니다!! 김부장이 2000원을 얻었습니다! >_< ");
	...
	public void mart() { 
		...
		System.out.println("-----------------------------------------");
		System.out.println("김부장이 가지고 있는 돈은 " + coin + " 원 입니다.");
		...
		if(number == 1) {
			...
			this.coin -= 4000;
			this.hp += 40;
			System.out.println("김부장이 4000원을 사용하고 상쾌한을 먹습니다. 김부장의 체력이 " + "40" + "% 충전되었습니다.");
			
			if(this.hp > 100) {
				  this.hp = 100;
			}
			
			System.out.println("김부장의 남은 돈은 " + coin + "원 입니다.");
			System.out.println("김부장의 체력은 " + hp + "%입니다.");
		}
		if(number == 2) {
			...
			this.coin -= 3000;
			this.hp += 30;
			System.out.println("김부장이 3000원을 사용하고 상쾌한을 먹습니다. 김부장의 체력이 " + "30" + "% 충전되었습니다.");
		...
		if(number == 3) {
			...
			this.coin -= 5000;
			this.hp += 50;
			System.out.println("김부장이 5000원을 사용하고 여명808을 먹습니다. 김부장의 체력이 " + "50" + "% 충전되었습니다.");
		...
		if(number == 4) {
			...
			this.coin -= 1000;
			this.hp += 10;
			System.out.println("김부장이 1000원을 사용하고 포카리를 먹습니다. 김부장의 체력이 " + "10" + "% 충전되었습니다.");
